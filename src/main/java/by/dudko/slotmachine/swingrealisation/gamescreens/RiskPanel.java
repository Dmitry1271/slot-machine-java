...
import static by.dudko.slotmachine.constants.RiskPanelConstants.*;

/**
 * Created by cplus on 01.10.2017.
 */
public class RiskPanel extends JPanel {
    private JLabel panelName = new JLabel(PANEL_NAME);
    private JLabel winningNow = new JLabel(WINNING_NOW_TEXT);
    private JLabel winningNext = new JLabel(WINNING_NEXT_TEXT);
    private JLabel wonValueNow = new JLabel(String.valueOf(GameTable.wonMoney));
    private JLabel wonValueNext = new JLabel(String.valueOf(LabelReader.getValue(wonValueNow) * 2));
    ...
    private void redBlackButtonsFunction(int value) {
        if (valueForSelection == value) {
            ...
            int result = JOptionPane.showConfirmDialog(null, CONFIRM_DIALOG_TEXT_AFTER_WON);

            if (result != JOptionPane.YES_OPTION || wonNumber == 5) {
                JOptionPane.showMessageDialog(null, MESSAGE_DIALOG_TEXT_AFTER_WON + LabelReader.getValue(wonValueNow));
                ...
                if (!MainScreen.isDemoGame) {
                    Login.user.setBalance(GameTable.balanceValue);
                }

                MainScreen.layout.show(MainScreen.mainPanel, "GameTable");
            }
        } else {
            JOptionPane.showMessageDialog(null, MESSAGE_DIALOG_TEXT_AFTER_LOSE + winningForFirstRisk + "!");
